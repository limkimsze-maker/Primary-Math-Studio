...
Addition moves the lower-row tokens into the upper row, visually groups exchanged tokens, creates the carried token in the adjacent column and reveals the matching written digit. Subtraction keeps one working row, shows every adjacent borrow separately, temporarily marks the donor and removed tokens, and updates the crossed-out/renamed written digits before revealing the result digit. Each click now plays slow, labelled movement stages with optional original Web Audio cues. Token crosses clear when the movement settles; crossed-out written digits remain as the mathematical record.
...
- Use the latest `main` branch of `limkimsze-maker/Primary-Math-Studio`.
- Live GitHub Pages studio: https://limkimsze-maker.github.io/Primary-Math-Studio/
- The repository root contains the static 15-engine selector required by GitHub Pages. Do not remove `index.html`; without it, GitHub Pages falls back to the README/development page.
- Use Expand workspace in the studio and Pupil view inside an engine for projector display.
