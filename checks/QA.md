...
Projector-ready Money teaching and public homepage

- Enlarged the Money place-value mat, note/coin diagrams, written algorithm, instruction card and controls on wide classroom displays. Direct teacher and pupil views fitted at 1363 × 936 with equal document scroll/client heights.
- One Next step now plays a locked, slow sequence. Addition pauses for move → group → exchange → record; subtraction pauses for donor → exchange → record or remove → record. Browser checks observed 6 moving 10¢ tokens, a group of 10 and one carried $1 token in $12.75 + $8.60.
- Added gentle Web Audio cues generated by the page for movement, grouping, exchange/borrowing, removal and recording. Sound can be switched on or off; no recorded or copied audio asset is used.
- Crosses on notes and coins are temporary action markers only. Browser checks observed one donor cross during borrowing and nine removal crosses during 16 − 9, then zero token crosses after each movement and on the completed $10.70 answer. Crossed-out written digits remain to preserve the renaming record.
- Added a root `index.html` for GitHub Pages. The previous repository README/development page is replaced by a live 15-engine tabbed studio with teacher guide, selected-engine URL, iframe workspace and Expand workspace mode. `checks/check-hub.mjs` verifies all engine tabs, routes, expanded control and credit.
