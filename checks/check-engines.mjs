...
known('fraction','write',{a:3,den:8},[3,8]);
known('fraction','like-add',{a:3,b:2,den:8},[5,8]);
known('fraction','like-subtract',{a:5,b:5,den:8},[0,1]);
known('fraction','unit-compare',{den:3,den2:5,orderMode:'compare'},'>');
...
known('fraction','unlike-add',{a:1,den:2,b:1,den2:4},[3,4]);
known('fraction','simplify',{a:4,den:8},[1,2]);
known('area','compare',{a:6,b:4,cols2:8,rows2:3},'Second is greater');
known('graph','total',{values:'12, 8, 16',key:2},36);
const frac=lesson(validate({...defaults('fraction',3,'simplify'),a:4,den:8}));assert(checkAnswer(frac,[1,2]));assert(!checkAnswer(frac,[2,4]));assert(!checkAnswer(frac,[1,0]));assert(!checkAnswer(frac,['',4]));
...
assert.deepEqual(tasks('fraction',2).map(x=>x[0]),['write','unit-compare','like-compare','like-add','like-subtract']);
assert.deepEqual(tasks('fraction',3).map(x=>x[0]),['write','unit-compare','like-compare','like-add','like-subtract','equivalent','simplify','unlike-compare','unlike-add','unlike-subtract']);
for(const grade of [2,3])for(const [task] of tasks('fraction',grade)){const l=lesson(validate(defaults('fraction',grade,task)));assert(Array.isArray(l.data.steps)&&l.data.steps.length>=3,`${task} needs clear teaching steps`);}
const ordered=lesson(validate({...defaults('fraction',3,'unlike-compare'),orderMode:'ascending',a:1,den:2,b:1,den2:4,cnum:3,den3:8}));assert.equal(ordered.answer,'1/4 < 3/8 < 1/2');assert(checkAnswer(ordered,ordered.answer));
...
assert.throws(()=>validate({...defaults('fraction',3,'like-add'),a:7,b:3,den:8}));
