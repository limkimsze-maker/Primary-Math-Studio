...
vm.runInContext(`${source.slice(start,end)}\nthis.moneyAPI={MONEY_DESC,moneyPlaceCounts,moneyAdditionPlan,moneySubtractionPlan,moneyPlanState,moneyAnimatedState,moneyAnimationText,moneyActionPhases,moneyOperationMat,moneyWrittenAlgorithm,moneyOperationStepPanel,moneyTransitionStrip};`,context);
const {MONEY_DESC,moneyPlaceCounts,moneyAdditionPlan,moneySubtractionPlan,moneyPlanState,moneyAnimatedState,moneyActionPhases,moneyOperationMat,moneyWrittenAlgorithm,moneyOperationStepPanel}=context.moneyAPI;
...
assert.deepEqual(plain(moneyActionPhases(add.actions[1])),[['move',3000],['group',3600],['exchange',4000],['record',2000]],'A regrouping step must pause slowly at every visible stage.');
assert(moneyOperationMat(add,moneyPlanState(add,2),2,null,true).includes('money-regrouped-glow'),'A newly carried money token must glow after it settles.');
const addMove=moneyAnimatedState(add,1,'move');
assert.deepEqual(plain(vector(addMove.top)),[0,1,2,13,1]);
assert.deepEqual(plain(vector(addMove.bottom)),[0,0,8,0,0]);
assert.equal(addMove.result.c10,null,'The digit must wait until the record stage.');
assert(moneyOperationMat(add,addMove,1,{step:1,phase:'move'}).includes('money-move-up-token'));
assert(moneyOperationMat(add,moneyAnimatedState(add,1,'group'),1,{step:1,phase:'group'}).includes('money-group-token'));
const addExchange=moneyAnimatedState(add,1,'exchange');
assert.equal(addExchange.top.c10,3);
assert.equal(addExchange.top.d1,3);
assert.equal(addExchange.result.c10,null);
assert(moneyOperationMat(add,addExchange,1,{step:1,phase:'exchange'}).includes('money-carry-token'));
assert.equal(moneyAnimatedState(add,1,'record').result.c10,3);
...
assert.deepEqual(plain(moneyActionPhases(subtract.actions[1])),[['donor',3000],['exchange',4200],['record',2100]],'Borrowing must pause slowly on the donor, exchange and written rename.');
assert(moneyOperationMat(subtract,moneyPlanState(subtract,2),2,null,true).includes('money-regrouped-glow'),'Newly renamed smaller money tokens must glow after they settle.');
const donorState=moneyAnimatedState(subtract,1,'donor');
assert.deepEqual(plain(vector(donorState.work)),[0,3,1,6,0]);
assert(moneyOperationMat(subtract,donorState,1,{step:1,phase:'donor'}).includes('money-donor-token'));
const borrowExchange=moneyAnimatedState(subtract,1,'exchange');
assert.deepEqual(plain(vector(borrowExchange.work)),[0,3,0,16,0]);
assert(moneyOperationMat(subtract,borrowExchange,1,{step:1,phase:'exchange'}).includes('money-borrow-new-token'));
...
const removeState=moneyAnimatedState(subtract,2,'remove');
assert.equal(removeState.result.c10,null);
assert.equal(removeState.work.c10,7);
assert(moneyOperationMat(subtract,removeState,2,{step:2,phase:'remove'}).includes('money-remove-slow-token'));
assert.equal(moneyAnimatedState(subtract,2,'record').result.c10,7);
...
const settledSubtraction=moneyOperationMat(subtract,moneyPlanState(subtract,subtract.actions.length),subtract.actions.length,null,true);
assert(!settledSubtraction.includes('removed-token'),'Crosses on removed notes or coins must disappear after the movement finishes.');
