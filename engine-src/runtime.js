...
function p1OperationReference(c){
 ...
 if(c.task==='count-on-back')return {title:'Addition / subtraction within 10 · Count on / count back',url:'https://limkimsze-maker.github.io/P1_Addition_Subtraction_Within_10_Count_On_or_Count_Backward/'};
 if(c.task==='fact-family')return {title:'Fact family',url:'https://limkimsze-maker.github.io/P1_Fact_Family/'};
 return null;
}
function showP1OperationReference(c){
 ...
 $('question').textContent=ref.title;
 $('taskLabel').textContent='Primary 1 · Operations';
 $('questionMeta').textContent='';
 $('activityInstruction').textContent='Use the original activity below.';
 $('feedback').textContent='';$('hint').hidden=true;
 $('answerForm').hidden=true;$('checkButton').hidden=true;$('hintButton').hidden=true;$('nextButton').hidden=true;
 const host=$('diagram');host.innerHTML=`<iframe id="p1OperationReferenceFrame" class="p1-operation-reference-frame" src="${ref.url}" title="${E(ref.title)}" loading="eager"></iframe>`;
 const frame=$('p1OperationReferenceFrame');
 const fit=()=>{try{const doc=frame.contentDocument;if(!doc)return;const h=Math.max(900,doc.documentElement?.scrollHeight||0,doc.body?.scrollHeight||0);frame.style.height=Math.ceil(h+4)+'px';}catch{}};
 frame.addEventListener('load',()=>{fit();setTimeout(fit,160);setTimeout(fit,600);try{const doc=frame.contentDocument;if(doc?.body&&'ResizeObserver'in window){const ro=new ResizeObserver(()=>requestAnimationFrame(fit));ro.observe(doc.body);frame._p1OperationObserver=ro;}}catch{}});
 return true;
}
...
function populateFields(){draft.mode=$('mode').value;draft.grade=Number($('grade').value);draft.task=$('task').value;$('engineFields').replaceChildren();const exploring=engine==='place'&&draft.task==='hundred',referenceMoneyWord=engine==='money'&&draft.task==='word',referenceP1Operation=Boolean(p1OperationReference({...draft,engine})),random=draft.mode==='random';$('mode').closest('.field').hidden=exploring||referenceMoneyWord||referenceP1Operation;$('countField').hidden=!random||exploring||referenceMoneyWord||referenceP1Operation;
 ...
 $('randomNote').textContent=referenceP1Operation?'This opens your original P1 activity inside Primary Maths Studio, with its original teaching flow and checking.':exploring?'Explore numbers from 0 to 100. Generate restores your starting number and both button amounts.':random?'New questions use this grade’s preset ranges. The settings shown here stay fixed; other numbers or shapes vary. Generate again or reopen the downloaded file for a new practice set.':'Your chosen numbers make one fixed example. Reopening or restarting keeps that same example.';
}
